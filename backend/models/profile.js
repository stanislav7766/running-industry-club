...
const profileSchema = new Schema({
  user: {
    ...
    nickname: {type: String, ref: 'users'},
  ...
  bookedRuns: [
    {
      nameRun: {type: String, required: true},
      distance: {type: String, required: true},
      status: {type: String, required: true},
      locationRun: {type: String, required: true},
...
profileSchema.statics.updateProfile = async function(profile) {
  return await this.findOneAndUpdate({'user.id': profile.user.id}, {$set: profile}, {new: true});
};
